...
export class MailService {
    ...
    async sendMedicalEventNormal(parentEmail: string, parentName: string, studentName: string, description: string, treatment: any) {
        // Đường dẫn đến template pug
        const templatePath = join(process.cwd(), 'src/configs/template/medical-event-normal.pug');
        // const templatePath = join(__dirname, '../../../configs/template/parent-registration.pug');
        ...
        // Gửi mail
        return this.sendMail(parentEmail, `Thông báo xảy ra sự kiện y tế em ${studentName}`, html);
    }
