...
@ApiTags(`${resourcesV1.Parent.root} - ${resourcesV1.Parent.REJECT_BENEFIT_MEDICINE_REQUEST.parent}`)
@Controller(routesV1.versionParent)

export class RejectBenefitMedicineRequestParentController {
    constructor(
    ) { }
    @ApiOperation({ summary: resourcesV1.Parent.REJECT_BENEFIT_MEDICINE_REQUEST.displayName })
    @ApiBearerAuth()
    @ApiParam({
        name: "id",
        description: "ID đơn gửi thuốc từ chối đề xuất",
        example: 1,
        type: Number
    })
    @UseGuards(JWTGuard, RolesGuard)
    @Roles(4)
    @Put(routesV1.parent.medicineRequest.rejected)
    async rejectBenefit(@Param('id') id: string, @GetUser() user) {
    }
}
